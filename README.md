...
- ドラッグ&ドロップ移動（フォルダ・サイドバー・パスバー・ツリー展開行へ。ゴミ箱へドロップで削除）
  — タッチは長押し、**マウスはプレス＋ムーブで即ドラッグ**（Finder 同様）
- **マウス / キーボードの複数選択**: 空白ドラッグのラバーバンド選択、Ctrl(/Cmd)+クリックでトグル、
  Shift+クリックで範囲選択、空白クリックで選択解除
- リスト表示の**列幅をヘッダ境界のドラッグで調整**（設定として永続化）
...
### APK のインストール
- Quick Look で APK を開くと、アイコン・バージョン・権限を確認してそのままインストール / アップデート
- ダウングレード・署名不一致・非対応 Android などは理由を表示。zip 内やネットワーク上の APK にも対応

### システム統合
- 他アプリの「ファイルを選択」（ACTION_GET_CONTENT）のピッカーとして利用可能
- **システムのファイルピッカーに dango が場所として表示**（DocumentsProvider）
- フォルダの ACTION_VIEW を受けて該当フォルダで起動
- サイドバーのクラウドリンク（Dropbox / Google ドライブ / Google フォト → 公式アプリへ遷移）

### その他
- macOS Finder 風の独自フォルダ・書類アイコン、NSMenu 風メニュー、NSAlert 風ダイアログ
- Quick Look は押したアイテムから拡大し、下スワイプで閉じる。削除した項目はゴミ箱へ吸い込まれる
